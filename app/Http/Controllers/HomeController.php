...
class HomeController extends Controller
{
    ...
    public function member_listing(Request $request)
    {
        ...
        if(Auth::user()->approved == 0){
            flash(translate('To view Active Members, Please complete your profile and await Admin approval'))->error();
        ...
        $diet_type = ($request->diet_type != null) ? $request->diet_type : null;
      ...
      // Gender Check
       ...
       // Search by Member Code
        ...
        // Sort by Diet Type
        if($diet_type != null){
            $user_ids = Member::where('diet_type_id', $diet_type)->pluck('user_id')->toArray();
            if(count($user_ids) >0){
                $users = $users->WhereIn('id', $user_ids);
            }
        }
    ...
    public function update_email_new(Request $request)

    {
        ...
        $messages = [
            'email.email'     => translate('Invalid email id.'),
        ];
        ...
        if ($validator->fails()) {
            flash(translate('Invalid email id.'))->error();
            return back()->with(['nextStep' => '1'])->withErrors($validator);
        }
        ...
        $user->email = $email;
        $user->save();
        if(User::where('email',$email)->where('id','!=',Auth::user()->id)->count() > 1 ){
            flash(translate('Warning: This email is already been used by another member profile.'))->warning();
        }
        else{
            flash(translate('Email Address has been updated successfully.'))->success();
        }
...

    {
        ...
        $array['content'] = 'This email is sent to you from Soulmate Matrimonial Services to verify your account, Verify your account by clicking below button';
    ...
    public function email_change_callback(Request $request){
        ...
        if($request->has('new_email_verificiation_code') && $request->has('email')) {
            ...
            if($user != null) {
                ...
                $user->email_verified_at1 = date('Y-m-d h:m:s');
