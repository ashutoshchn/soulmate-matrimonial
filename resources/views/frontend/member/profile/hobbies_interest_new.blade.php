<div class="card">
    ...
    <div class="card-body">
      <form action="{{ route('hobbies.update_new', $member->id) }}" method="POST" id="regForm-11">
          <input name="_method" type="hidden" value="PATCH">
          @csrf
          <div class="form-group row">
              <div class="col-md-6">
                  <label for="hobbies">{{translate('Hobbies')}}</label>
                  <input type="text" name="hobbies" value="{{ !empty($member->hobbies->hobbies) ? $member->hobbies->hobbies : "" }}" class="form-control" placeholder="{{translate('Please write about your Hobbies')}}">
              </div>
              <div class="col-md-6">
                  <label for="interests">{{translate('Interests')}}</label>
                  <input type="text" name="interests" value="{{ !empty($member->hobbies->interests) ? $member->hobbies->interests : "" }}" placeholder="{{ translate('Please write about your Interests') }}" class="form-control">
              </div>
          </div>
     <!--     < div class="form-group row">
              <div class="col-md-6">
