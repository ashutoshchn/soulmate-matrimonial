...
<div class="tab">
   <div class="card" id="introduction">
      ...
      <div class="card-body">
         <form action="{{ route('member.introduction.update', $member->member->id) }}" id="regForm-1" method="POST">
            @csrf
            <div class="form-group row">
            ...
            <div class="badge-soft-primary fs-12"><strong>

                Before you proceed, please make sure to consider updating “Gallery” with at least two photographs with one close up/face photo and one full size photo from <a href="{!! url('/gallery-image'); !!}">here</a>

            </strong>
            </div>
            <div class="form-group mb-0 text-right">
               @if(session()->has('nextStep'))
               <p id="nextStep-{{session('nextStep')}}"></p>
               @endif
            </div>
         </form>
      </div>
...
<div class="tab">
   <div class="card" id="email">
      ...
      <div class="card-body">
         <form action="{{ route('user.change.email_new') }}" id="regForm-2" method="POST">
            @csrf
            <div class="row">
               <div class="col-md-2">
...
<div class="tab">
   ...
   @include('frontend.member.profile.basic_info_new')
</div>
<!-- Present Address -->
<div class="tab">
   @php
   ...
   @include('frontend.member.profile.present_address_new')
   @endif
</div>
...
<div class="tab">
   ...
   @include('frontend.member.profile.residency_information_new')
   @endif
</div>
<!-- Permanent Address -->
<div class="tab">
   ...
   @include('frontend.member.profile.permanent_address_new')
   @endif
</div>
<!-- Education -->
<div class="tab">
   ...
   @include('frontend.member.profile.education.index_new')
   @endif
</div>
<!-- Career -->
<div class="tab">
   ...
   @include('frontend.member.profile.career.index_new')
   @endif
</div>
<!-- Physical Attributes -->
<div class="tab">
   ...
   <div id="physical" ></div>
   @include('frontend.member.profile.physical_attributes_new')
   @endif
</div>
<!-- Language -->
<div class="tab">
   ...
   @include('frontend.member.profile.language_new')
   @endif
</div>
<!-- Hobbies  -->
...
<div class="tab">
   ...
   @include('frontend.member.profile.hobbies_interest_new')
</div>
@endif
...
<div class="tab">
   ...
   @include('frontend.member.profile.spiritual_backgrounds_new')
   @endif
</div>
<!-- Life Style -->
<div class="tab">
   ...
   @include('frontend.member.profile.lifestyle_new')
   @endif
</div>
...
<div class="tab">
   ...
   @include('frontend.member.profile.astronomic_information_new')
   @endif
</div>
<!-- Family Information -->
<div class="tab">
   ...
   @include('frontend.member.profile.family_information_new')
   @endif
</div>
<!-- Partner Expectation -->
<div class="tab">
   ...
   @include('frontend.member.profile.partner_expectation_new')
   @endif
</div>
<div class="tab">
   @if(Auth::user()->approved == 0)
   <div class="card" id="last">
      ...
      <div class="card-body">
         <form action="{{ route('member.send_admin_approval_new', $member->member->id) }}" method="POST" id="regForm-17">
            @csrf
            <div class="form-group row">
      ...
      <div class="card-body">
            <div class="form-group row">
               <div class="badge-soft-primary">
                  {{translate('Thank you, we have received your approval request, please wait for our decision or contact us directly.')}}
               </div>
            </div>
      </div>
   </div>
...
<div style="overflow:auto;">
   <div style="float:right;">
    ...
    @if(session()->has('nextStep') && session('nextStep')!=17)

      <button type="button" id="prevBtn" class="btn btn-primary btn-sm" onclick="prev(-1)">Previous </button>


    @endif
    @if(session('nextStep')!=16 && session('nextStep')!=17)
      <button type="button" id="nextBtn" class="btn btn-primary btn-sm" onclick="next(1)">Next  </button>
    @endif  
   </div>
</div>
...
<script type="text/javascript">
   ...
   function education_add_modal(id){
    ...
    //  education Add edit , status change
    function education_add_modal_new(id){
      $.post('{{ route('education.create_new') }}',{_token:'{{ @csrf_token() }}', id:id}, function(data){
          $('.create_edit_modal_content').html(data);
          $('.create_edit_modal').modal('show');
      });
   }
   ...
   function education_edit_modal_new(id){
       $.post('{{ route('education.edit_new') }}',{_token:'{{ @csrf_token() }}', id:id}, function(data){
           $('.create_edit_modal_content').html(data);
           $('.create_edit_modal').modal('show');
       });
   }
   ...
   function career_add_modal_new(id){
      $.post('{{ route('career.create_new') }}',{_token:'{{ @csrf_token() }}', id:id}, function(data){
          $('.create_edit_modal_content').html(data);
          $('.create_edit_modal').modal('show');
      });
   }
   ...
   function career_edit_modal_new(id){
       $.post('{{ route('career.edit_new') }}',{_token:'{{ @csrf_token() }}', id:id}, function(data){
           $('.create_edit_modal_content').html(data);
           $('.create_edit_modal').modal('show');
       });
   }
   ...
   @if (session()->has('nextStep')) 
           currentTab={{session()->get('nextStep')}};
       @endif
   ...
   function next(n) {
        ...
        var y = x[currentTab].getElementsByTagName("form");
       currentTab = currentTab + n;
       console.log(currentTab);
        // if you have reached the end of the form... :
        if (currentTab <= x.length) { 
        //...the form gets submitted:
            document.getElementById("regForm-"+currentTab).submit();
            return false;
        }
   }
   function prev(n) {
