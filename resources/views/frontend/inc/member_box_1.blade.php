	...
	@endphp
		...
		<div class="absolute-bottom-left w-100 p-3 z-1">
			...
			<div class="text-center">
				<div class="text-primary fw-500 mb-1">
				<span class="opacity-60">{{ translate('Member ID: ') }}</span>{{ $member->code }}
				</div>
            <div class="fs-10">
				<span class="opacity-60">{{ translate('Age: ') }}</span><span class="ml-2 text-primary">{{ \Carbon\Carbon::parse($member->member->birthday)->age }}</span>
            </div>
			</div>
		</div>
	</a>
