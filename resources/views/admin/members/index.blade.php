...
<div class="aiz-titlebar mt-2 mb-4">
    <div class="row align-items-center">
        <div class="col-md-6">
        @php if($members->count()>0) $firstMember = \App\Models\Member::where('user_id',$members->first()->id)->first() @endphp
            @php  if($members->count()>0 && isset($firstMember)) $package = \App\Models\Package::where('id',$firstMember->current_package_id)->first() @endphp                          
                
            @if($members->count()>0  && isset($firstMember)) 
            <h1 class="h3"><strong>{{ $members->count()>0 && isset($package) ? $package->name : ''}} {{translate('Members')}}</strong></h1>
            @else
            <h1 class="h3"><strong>{{translate('Members')}}</strong></h1>
            @endif
        </div>
        @can('create_member')
        ...
        <div class="card">
            <div class="card-header row gutters-5">
  				<div class="col text-center text-md-left">
  					<h5 class="mb-md-0 h6">All {{$members->count()>0  && isset($firstMember) ? $package->name: ''}} Members</h5>
  				</div>
  				<div class="col-md-3">
                    ...
                    <tbody>
                        @foreach($members as $key => $member)
                            <tr>
                                ...
                                @else
                                    <td><span class="badge badge-inline badge-warning">Approval Status: {{translate('Pending')}}</span></td>
                                @endif
                                <td>
                                ...
                                <td>
                                    @if($member->deactivated == 0)
                                        <span class="badge badge-inline badge-success">Member's Status: {{translate('Active')}}</span>
                                    @else
                                        <span class="badge badge-inline badge-danger">Member's Status: {{translate('Deactivated')}}</span>
                                    @endif
                                </td>

                                <td>
                                @php $memberTable = \App\Models\Member::where('user_id',$member->id)->first() @endphp
                                @if(isset($memberTable))
                                    @php $package = \App\Models\Package::where('id',$memberTable->current_package_id)->first() @endphp                          
                                    {{$package->name}}
                                @endif
                                </td>
                                <td>
