        ...
        <div class="card">
            <div class="card-body text-center">
                ...
                <div class="pad-ver btn-groups">
    				<a href="javascript:void(0);" onclick="package_info({{$member->id}})" class="btn btn-info btn-sm add-tooltip">{{ translate('Package') }}</a>
                    @can ('approve_member')
                        @if($member->approved == 0)
                            <a href="javascript:void(0);" onclick="approve_member({{$member->id}})" class="btn btn-primary btn-sm add-tooltip" >{{translate('Approve')}}</a>
                        @endif
                    @endcan

                    @if($member->blocked == 0)
                        <a href="javascript:void(0);" onclick="block_member({{$member->id}})" class="btn btn-dark btn-sm add-tooltip">{{ translate('Block') }}</a>
                    @elseif($member->blocked == 1)
                        <a href="javascript:void(0);" onclick="unblock_member({{$member->id}})" class="btn btn-dark btn-sm add-tooltip">{{ translate('Unblock') }}</a>
                    @endif
                    <br><br>
                    @if($member->approved == 1)
                        <span class="badge badge-inline badge-success">Approval Status: {{ translate('Approved') }}</span>
                    @elseif($member->blocked == 1)
                        <span class="badge badge-inline badge-danger">Approval Status: {{ translate('Blocked') }}</span>
                    @else
                        <span class="badge badge-inline badge-warning">Approval Status: {{translate('Pending')}}</span>
                    @endif
                    @if($member->deactivated == 0)
                        <span class="badge badge-inline badge-success">Member's Status: {{translate('Active Account')}}</span>
                    @else
                        <span class="badge badge-inline badge-danger">Member's Status: {{translate('Deactivated Account')}}</span>
                    @endif
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <tr>
                    ...
                    <td>{{ !empty($member->member->birthday) ? date('d-m-Y', strtotime($member->member->birthday)) : ''}}</td>
                </tr>
                <tr>
            ...
            <div class="card-body">
                <table class="table">
                    <tr>
                        ...
                        <td>{{ !empty($member->physical_attributes->height) ? number_format((float)$member->physical_attributes->height,2) : "" }}</td>
              ...
              <div class="card-body">
                  <table class="table">
                      ...
                      <tr>
                          ...
                          <td>{{ !empty($member->recidency->immigration_status) ? translate($member->recidency->immigration_status) : "" }}</td>
                      </tr>
                  </table>
              </div>
          </div>
              ...
              <div class="card-body">
                  <table class="table">
                      ...
                      <tr>
                          ...
                          <td>{{ !empty($member->spiritual_backgrounds->family_value) ? $member->spiritual_backgrounds->family_value : "" }}</td>
                      <!-- </tr>
                      <tr>
              ...
              <div class="card-body">
                  <table class="table">
  	                 <tr>
                          ...
                          <td>
                            @if(!empty($member->lifestyles->diet))
                              {{ App\Models\DietType::where('id',$member->lifestyles->diet)->first()->name }}
                            @endif
                        </td>   

                          <th>{{translate('Drink')}}</th>
                          <td>{{ !empty($member->lifestyles->drink) ? translate($member->lifestyles->drink) : "" }}</td>
                      </tr>
                      <tr>
                          <th>{{translate('Smoke')}}</th>
                          <td>{{ !empty($member->lifestyles->smoke) ? translate($member->lifestyles->smoke) : "" }}</td>

                          <th>{{translate('Pet')}}</th>
                          <td>{{ !empty($member->lifestyles->living_with) ? translate($member->lifestyles->living_with) : "" }}</td>
                      </tr>
                  </table>
              </div>
          </div>
              ...
              <div class="card-body">
                  <table class="table">
                      ...
                      <tr>
                          ...
                          <td>{{ !empty($member->astrologies->manglik) ? $member->astrologies->manglik : "" }}</td>
              ...
              <div class="card-body">
                  <table class="table">
                      ...
                      <tr>
                          ...
                          <td> @if(!empty($member->partner_expectations->language_id))
                                         {{ optional(\App\Models\MemberLanguage::where('id',$member->partner_expectations->language_id)->first())->name }}
                                                                @endif</td>
                      </tr>
                      ...
                      <tr>
                          ...
                          <td>{{ !empty($member->partner_expectations->smoking_acceptable) ? translate($member->partner_expectations->smoking_acceptable) : "" }}</td>

                          <th>{{translate('Drinking Acceptable')}}</th>
                          <td>{{ !empty($member->partner_expectations->drinking_acceptable) ? translate($member->partner_expectations->drinking_acceptable) : "" }}</td>
                      </tr>
                      <tr>
                          <th>{{translate('Diet')}}</th>
                          <td>@if(!empty($member->partner_expectations->diet))
                                         {{ optional(\App\Models\DietType::where('id',$member->partner_expectations->diet)->first())->name }}
                                                                @endif
                              
                        </td>
                        <th>{{translate('Manglik')}}</th>
                          <td>{{ !empty($member->partner_expectations->manglik) ? translate($member->partner_expectations->manglik) : "" }}</td>
